...
public class PreVo_Server extends JFrame {
	private ArrayList<MultiServerThread> clientList; // ArrayList 는 Linked List
														// 로 구현된 배열 // 선언방법 ->
														// ArrayList<자료형> 변수이름
	...
	public PreVo_Server() {
		...
		try {
			ServerSocket serverSocket = new ServerSocket(6406); // 서버 소켓 생성
			...
			while (!isStop) {
				...
				mst.start(); // 쓰레드 시작
				inputArea.setText("현재 접속한 사용자수 : " + clientList.size());
			} // while
		} catch (IOException e) {
			e.printStackTrace();
		} // catch
	...
	class MultiServerThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				oos = new ObjectOutputStream(socket.getOutputStream()); // 데이터
																		// 보냄
				...
				while (!isStop) {
					...
					for (int i = 0; i < clientList.size(); i++) {
						viewArea.setText("접속한 사용자 : " + str[0]);
					}
				...
				viewArea.append(address + " IP 주소의 사용자께서 종료하셨습니다.\n");
				inputArea.setText("남은 사용자 수 : " + clientList.size());
			} catch (Exception e) {
				clientList.remove(this); // 장길산을 뺀다.
				viewArea.append(address + " IP 주소의 사용자께서 비정상 종료하셨습니다.\n");
				inputArea.setText("남은 사용자 수 : " + clientList.size());
			} // catch
		} // run
		...
		public void broadCasting(String message) {
			for (MultiServerThread ct : clientList) { // 향상된 for문, ct에 list의 원소를
														// 하나씩 저장 한다.
				ct.send(message);
			} // for
		} // broadCasting
